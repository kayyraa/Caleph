...
ImageInput.onchange = () => {
    Array.from(ImageInput.files).forEach(async (File) => {
        ...
        console.log(await new Api.GithubStorage(File).Download());
        location.reload();
    });
}
        ...
        if (FoundArticles.length === 0) {
            ...
            FoundArticles.forEach((Article) => {
                ...
                ArticleElement.addEventListener("click", Event => {
                    ...
                    } else if (!Api.Admins.includes(JSON.parse(localStorage.getItem("User")).Username)) ArticleContent.querySelector(".Bottombar").style.display = "none";
                });
